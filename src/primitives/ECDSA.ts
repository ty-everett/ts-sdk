...
export const verify = (msg: BigNumber, sig: Signature, key: Point): boolean => {
  ...
  const u1 = msg.mul(sinv).umod(curve.n)
  const u2 = r.mul(sinv).umod(curve.n)

  const p = curve.g.jmulAdd(u1, key, u2).toP()

  if (p.inf) return false

  const x = p.x ?? curve.zero
  const v = x.fromRed().umod(curve.n)
